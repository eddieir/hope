...
**Trigger tracking**  Not all cigarettes are the same, and treating a stress cigarette the same as a "smoked one out of habit while making coffee" cigarette makes for a worse plan. The dashboard has four action-first buttons — **"Log smoke 🚬"**, **"Log resisted craving 💪"**, **"Log delayed craving ⏳"**, and **"Start 10-min delay ▶"** — and the first three open a fast, chip-based flow instead of a form:

1. Tap what's going on — 15 chips, emotional ones grouped first: Stress, Anxiety/Panic, Anger, Sadness, After argument, Loneliness, then Boredom, After meal, Coffee/tea, Work break, Social/alcohol, Morning/after waking, Before sleep, Habit/automatic, and Other.
2. Tap how strong the urge is, 1–10.
3. If you smoked, tap how long you delayed first (immediate / 5 / 10 / 20 min). If you're logging a delayed craving, tap how long you delayed it (5 / 10 / 20 min), then say whether you ended up smoking anyway.
4. Optional one-line note, or skip it.

Each tap auto-advances to the next step, so the whole thing takes a few seconds, on purpose, because a stressed person mid-craving isn't going to fill out a form. Every trigger belongs to one of four groups that drive the rest of the plan:
- **Automatic** (habit, boredom) — low-awareness, autopilot smoking. Usually the easiest to cut first.
- **Ritual** (after meal, coffee/tea, work break, social/alcohol, morning/after waking, before sleep) — tied to a routine, cut once automatic ones are down.
...
- **Other** — doesn't fit a clear pattern yet; no cutting-order recommendation is made for it, it just keeps the stats honest instead of forcing an "other" cigarette into a bucket it doesn't belong in.

Each log is tagged with an explicit outcome (`smoked` / `resisted` / `delayed`) so "delayed" cravings — the ones where willpower and a coping trick were actually exercised — show up as their own stat instead of being folded into "resisted" or ignored. After you save a log, the app shows a message that matches the outcome: "Logged. This is data, not failure." or "A slip does not restart your progress." for a smoked one, "Notice the trigger and continue." or "You're learning your pattern." for a resisted one, "You delayed it — that still counts as progress." for a delayed one — plus a trigger-specific coping suggestion to try next time. Never "failed," "restart," or "you ruined your streak."

Recent activity shows the trigger and intensity right in the log line, e.g. `🚬 Smoked — Stress — intensity 8/10 — 18:42`, instead of a bare "smoked one at 6:42pm" — the whole point of tracking triggers is that they stay visible, not buried in a modal you already closed.

**Urge delay**  Two ways to use the delay tactic:
- **"Start 10-min delay ▶"** — starts a live countdown banner (it survives a page reload). While it's running you can end it early with "Craving passed 🎉" or "I smoked anyway." When it ends, one way or another, the app asks whether the craving reduced and what helped (chips pulled from the same trigger-specific coping list), then logs it with a `delayed` outcome.
- **"Log delayed craving ⏳"** — for logging a delay after the fact (you didn't have the timer running, but you held off anyway): pick the trigger and intensity, how long you delayed, and whether you ended up smoking.

**Your smoking patterns**  Once there's at least one logged cigarette or craving, a "Your smoking patterns" card appears on the dashboard with: top trigger today, top trigger this week, strongest craving trigger (highest average intensity), emotional/ritual/automatic cigarette counts, cravings resisted, cravings delayed, average craving intensity, average delay before smoking, automatic cigarettes reduced vs. the previous week, your best smoke-free time-of-day window, a "top triggers this week" bar breakdown, and a stacked 4-group progress bar. Above the stats sits a non-shaming coaching line that names whichever group dominates your week, e.g. *"Most of your cigarettes this week were stress-related. Don't attack these first — start by reducing automatic cigarettes and use delay/coping tools for stress smokes."* The taper/status card also gets a separate "Focus next" note recommending which group to target based on plan stage (automatic first, then ritual, emotional last, replaced with coping actions rather than sudden cold turkey).

**Update banner**  If you (or anyone using the live site) already have the app open when a new version deploys, a small "New version available / Reload" banner appears at the bottom of the screen once the new service worker activates in the background — tapping Reload picks up the update immediately instead of silently running a stale cached build. First-time visitors never see it, since there's nothing to update from yet.
...
**Crisis button**  A visually distinct **"I want to smoke right now 🆘"** button sits above the four regular action buttons on the dashboard, always visible, not just something you find after digging through a menu. Tapping it opens a calmer, narrower flow than the rest of the app — one coping suggestion picked from whichever trigger group has been logged most this week (the same logic behind "Focus next," not a separate recommendation engine), a one-tap "Start 10-min delay" button, and the same non-judgmental resisted/delayed/smoked logging options at the end. Fewer choices, bigger buttons, less text, on purpose: the moment you'd reach for this button is the worst possible moment to make someone read a paragraph or pick from a long list.

**Backup: export & import**  Your data lives only in this browser's `localStorage` (see "Known limitations" below), so Settings has an **"Export my data"** button that downloads everything, onboarding answers, the taper schedule, every log, settings, as a single `quitpath-backup-YYYY-MM-DD.json` file. An **"Import data"** button restores from that file on any browser/device: it checks the file actually looks like a QuitPath backup before touching anything, asks a plain yes/no question before overwriting what's currently on the device (no dark patterns), and if the file's not valid JSON or is missing expected fields, says so calmly instead of throwing a cryptic error.

**Feedback**  A "Feedback" card in Settings with a GitHub issue link and a `mailto` link, no form, no backend. The issue link pre-fills a bug report template (`.github/ISSUE_TEMPLATE/bug_report.md`) that asks what happened, steps to reproduce, browser/device, and whether any data was lost, so reports arrive with the context that actually matters for a client-only app.
...
- **Cigarette logs are timestamped events, not counters.** Each log entry has a timestamp, a legacy `type` (`cigarette` or `resisted`, with `slip` kept around for backward compatibility with earlier data), a boolean `smoked`, an explicit `outcome` (`smoked` / `resisted` / `delayed`), plus optional `triggerId`, `triggerLabel`, `triggerGroup`, `cravingIntensity` (1–10), `delayMinutes`, and `note` fields. `triggerLabel`/`triggerGroup` are snapshotted at log time rather than only derived live, so a log's meaning doesn't silently change if the trigger catalog is edited later. The current-day tally, the "since quit date" total, the recent-activity list, and the trigger analytics are all just filters/aggregations over that one array. Older logs (pre-refactor) used `trigger`/`intensity` instead of `triggerId`/`cravingIntensity` — every read goes through `getTriggerId()` / `getTriggerLabel()` / `getTriggerGroupOf()` / `getIntensity()` in `app.js`, which fall back to the old field names transparently, so historical data isn't silently dropped from the newer stats. `isDelayedOutcome()` similarly infers a `delayed` outcome from `delayMinutes` for logs saved before the explicit `outcome` field existed.
- **Trigger data lives in three places in `app.js`**: `TRIGGERS` (the 15 chip options, in display order, and which of 4 groups each belongs to — automatic / ritual / emotional / other), `COPING_SUGGESTIONS` (per-group replacement actions, including an `other` fallback list), and `GROUP_INFO` (label + description per group, used for both the "Focus next" and "Your smoking patterns" cards). `triggerGroup()` is the one function that maps a trigger id to its group; if you add or re-categorize a trigger, that's the only place you need to change it. `other` is a real 4th group (not folded into `automatic`) precisely so a genuinely-unclassified cigarette doesn't quietly skew the automatic/ritual/emotional cutting-order recommendation.
...
- **The service worker caches app shell assets for offline use**, and its `CACHE_NAME` is versioned (currently `quitpath-v12`). Bump it in the *same commit* as any change to `app.js`, `style.css`, or `index.html` — otherwise returning users keep getting served stale assets out of the cache instead of your update, and the update-available banner (see below) never fires because it only triggers off a genuine service-worker byte change. This bit me twice during development: once by shipping a color palette change without bumping it, and again by shipping a button-label change without bumping it — the fix both times was a one-line `CACHE_NAME` bump in a follow-up commit.
- **The update-available banner** (`showUpdateBanner()` in `app.js`) listens for the service worker's `updatefound` event and only shows the banner if `navigator.serviceWorker.controller` was already set at that moment — meaning some worker was already actively controlling the page. That's what distinguishes "a returning visitor's browser just detected a genuine new deploy" from "this is the very first-ever service worker install for a brand-new visitor," which should never show an update banner since there's nothing to update from.
- **The color palette is one block of CSS custom properties** at the top of `style.css` (`--primary`, `--success`, `--danger`, background/text/border tokens, both light and dark variants, plus `--series-1` through `--series-6` for the trigger/cigarette-type charts, reused for both the individual-trigger bars and the 4-group automatic/ritual/emotional/other bar). The series colors were run through a colorblind-safety and contrast validator before picking them, if you add more chart categories, re-validate rather than eyeballing a new hue. If you want to reskin this, that block is the only place you need to touch, nothing else hardcodes a color.
- **`saveData()` never throws.** `localStorage.setItem` can fail (quota exceeded, or a browser blocking storage entirely in some private-browsing configurations), and every log, delay-timer tick, and settings change goes through this one function, so it wraps the write in try/catch and shows a calm, dismissible banner ("this device isn't saving new changes right now...") instead of letting the failure bubble up into a broken button or blank screen. The app keeps working for the rest of the session either way; the data just won't survive a reload until storage is available again.
...
- **No cross-device sync.** Data is per-browser, per-device. Clearing site data deletes your history. This is a direct consequence of having no backend, and it's a deliberate trade-off, not an oversight. Export/Import (Settings) is the manual workaround, and onboarding now says so up front rather than leaving people to discover it after losing data.
...
- **"Your smoking patterns" needs some logging history to be meaningful.** The card itself appears after your very first log, but stats like "top trigger this week" only reflect the last 7 days, and "automatic cigs reduced" compares against the *previous* 7 days, so it shows `—` until there's two weeks of history to compare against.
